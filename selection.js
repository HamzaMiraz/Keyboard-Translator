...
function handleTextSelection() {
    ...
    if (text.length > 1 && isEditableArea(sel)) {
        ...
        if (typeof renderVSCodeSuggestion === 'function') {
        ...
        if (window.SmartAPI && window.SmartAPI.fetchSentenceTranslation) {

            window.SmartAPI.fetchSentenceTranslation(text).then(translation => {
